...
function togTask(id,el){const t=D.tasks.find(x=>x.id===id);if(!t)return;if(el&&el.checked!==undefined){t.done=el.checked;}else{t.done=!t.done;}if(t.done){t.doneAt=Date.now();celebrate();autoAddWin(t.text,t.date||todayStr());}else{delete t.doneAt;}save();setTimeout(()=>{renderCalendar();renderCalTasks();renderAllTasks();updateStats();},300);}
